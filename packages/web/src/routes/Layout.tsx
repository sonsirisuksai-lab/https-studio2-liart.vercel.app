  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-[var(--theme-accent,var(--theme-primary))]/5 blur-[150px] rounded-full animate-pulse-slow" style={{ animationDelay: '2s' }} />
        ...
        <div 
          ...
          style={{ 
            backgroundImage: realmId === 'blueprint' 
      ...
      <div className="fixed top-8 right-8 z-50 flex flex-col gap-3 items-end pointer-events-none">
        <Glass className="px-4 py-2 flex items-center gap-4 bg-black/20 backdrop-blur-xl border-white/5 pointer-events-auto">
          <div className="flex flex-col items-end">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40">Universe Core</span>
            <span className="text-sm font-mono font-bold text-[var(--theme-primary)]">{Math.floor(energy)}%</span>
          </div>
        ...
        <div className="flex items-center gap-2 pr-2">
          <div className={`w-2 h-2 rounded-full ${isUniverseStable ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 'bg-amber-500 animate-pulse shadow-[0_0_10px_#f59e0b]'}`} />
          <span className="text-[10px] font-black uppercase tracking-widest text-white/60">
            {universeState === 'shifting' ? 'Dimensional Shift' : universeState === 'active' ? 'Neural Processing' : 'System Stable'}
          </span>
        </div>
      </div>
      ...
      <AnimatePresence>
        {!isUniverseStable && (
          ...
          >
            ...
            <div className="absolute w-[60vw] h-[60vw] border-[1px] border-[var(--theme-primary)]/5 rounded-full animate-ping-slow opacity-10" style={{ animationDelay: '0.5s' }} />
          </motion.div>
        )}
      </AnimatePresence>
